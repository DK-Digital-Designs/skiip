...
| `OrderService.createOrder()` | `order-create` Edge Function | Already server-authoritative | Browser submits product ids, quantities, contact details, tip, and scheduled collection metadata. Function validates auth, loads products, computes subtotal, service fee, and total, checks inventory, and writes orders/items through `create_order_with_items_v1()`. |
