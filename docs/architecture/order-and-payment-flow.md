...
3. [`Checkout.jsx`](../../app/src/pages/attendee/Checkout.jsx) submits only product IDs, quantities, contact details, the required operational phone number, WhatsApp opt-in status, notes, tip, and an immediate-collection scheduled payload. The browser may display service fees, but it does not supply them authoritatively. Buyer-facing scheduled-order controls are currently hidden from checkout; backend validation/display support remains for existing or future scheduled orders.
